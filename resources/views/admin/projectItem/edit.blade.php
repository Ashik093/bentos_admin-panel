	  ...
	  <div class="container-fluid">
	    <!-- Small boxes (Stat box) -->
			<div class="card">
			  ...
			  <!-- /.card-header -->
			  			<div class="card-body">
			  				<form action="{{route('projectitem.update',$data->id)}}" method="post" enctype="multipart/form-data">
			  					@csrf
			  					<div class="row">
										   ...
										   <div class="form-group">
											...
											<select class="form-control @error('project_id') is-invalid @enderror" id="project_id" name="project_id">
												...
												@foreach($project as $row)
													<option @if($row->id == $data->project_id) selected @endif value="{{$row->id}}">{{$row->title}}</option>
												@endforeach
